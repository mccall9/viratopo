...
export default function Home() {
  ...
  return <main className="arena-app">
    ...
    <section className="arena-hero" id="arena">
      <div className="hero-copy"><span className="kicker">TEMPORADA EM PRÉ-LANÇAMENTO</span><h1>Faça seu produto<br /><em>merecer a tela.</em></h1><p>ViraTopo é uma arena pública para produtos independentes. A classificação só começa quando houver um pagamento confirmado.</p><div className="hero-actions"><button className="button button-primary" onClick={() => setFlow("entry")}>Entrar na arena <ArrowRight size={17} /></button><a className="button button-ghost" href="/ranking">Ver ranking <ChevronRight size={17} /></a></div></div>
      <aside className="hero-ranking" aria-label="Prévia do ranking"><div className="hero-ranking-top"><span>RANKING DA TEMPORADA</span><b>AO VIVO</b></div><div className="hero-ranking-head"><span>POS.</span><span>PRODUTO</span><span>LANCE</span></div><ol><li><b>01</b><span className="rank-available">aguardando primeiro produto</span><strong>—</strong></li><li><b>02</b><span className="rank-available">posição disponível</span><strong>—</strong></li><li><b>03</b><span className="rank-available">posição disponível</span><strong>—</strong></li></ol><div className="hero-ranking-bottom"><span>0 entradas confirmadas</span><a href="/ranking">Abrir ranking <ChevronRight size={14} /></a></div></aside>
    </section>
    <section className="command-grid" aria-label="Central de controle da arena">
      <article className="command-card main-command"><div><span className="card-label">PRÓXIMA JOGADA</span><h2>Coloque seu produto no ranking.</h2><p>Defina o primeiro lance. Você receberá o comprovante antes de qualquer cobrança.</p></div><button className="button button-light" onClick={() => setFlow("entry")}>Criar entrada <ArrowRight size={17} /></button></article>
    ...
    {flow && <div className="flow-backdrop" onMouseDown={() => setFlow(null)}><section className="flow-panel" role="dialog" aria-modal="true" aria-labelledby="flow-title" onMouseDown={(event) => event.stopPropagation()}><button className="icon-close" onClick={() => setFlow(null)} aria-label="Fechar"><X size={19} /></button>
      {flow === "entry" && <form onSubmit={submitEntry}><span className="card-label">NOVA ENTRADA</span><h2 id="flow-title">Entrar no<br /><em>ranking.</em></h2><label>URL DO PRODUTO<input value={url} onChange={(event) => setUrl(event.target.value)} required autoFocus inputMode="url" placeholder="seuproduto.com" /></label><div className="bid-row"><span>LANCE INICIAL</span><div><button type="button" onClick={() => setBid((value) => Math.max(1, value - 1))} aria-label="Diminuir lance"><Minus size={16} /></button><strong>R$ {bid},00</strong><button type="button" onClick={() => setBid((value) => value + 1)} aria-label="Aumentar lance"><Plus size={16} /></button></div></div><button className="button button-primary" type="submit">Continuar <ArrowRight size={17} /></button></form>}
